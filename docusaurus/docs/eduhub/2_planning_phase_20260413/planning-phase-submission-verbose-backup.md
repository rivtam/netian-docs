...
4. **Centralize All Data**: Single database replacing the fragmented Moodle + iEnabler + paper forms environment
...
The problem: Students have to juggle three different systems. Course registrations in myAdmin may not immediately reflect in myModules, suggesting synchronization delays. Lecturers likely need to check multiple systems for student information. While sharing login credentials, each system has its own interface and user experience.
...
3. **Major Platforms All Have Limitations**:
   ...
   - **iEnabler**: thorough SIS, but NO LMS capabilities
...
From our research, the following technologies make sense for this project:

- **Frontend**: React.js (component-based, widely supported, large environment)
- **Backend**: Node.js with Express (JavaScript full-stack, non-blocking I/O (handles many requests at once), scalable)
- **Database**: PostgreSQL (reliable, ACID compliant (ensures data integrity), supports complex queries)
...
1. **Start with Authentication**: Secure authentication is the foundation - build properly from the beginning
...
- **Sequelize**: ORM - basically a way to talk to the database using JavaScript objects instead of SQL for database operations
...
| Risk                                     | Probability | Impact | How We'll Handle It                                     |
| ---------------------------------------- | ----------- | ------ | ------------------------------------------------------- |
| Learning curve for new technologies      | Medium      | Low    | Allocate time for tutorials and documentation           |
| Database performance with large datasets | Low         | Medium | build indexing, query optimization                  |
...
- **Caching**: Can build Redis for session storage and caching
...
| System downtime costs              | Low         | R100-500/incident     | build backups, monitoring, quick recovery procedures |
...
- **Stakeholder Engagement**: Regular demonstrations keep people involved informed and involved
- **Team Collaboration**: Daily communication and shared responsibility improve productivity

### Agile Principles Applied to EduHub

1. **Working Software**: Prioritize functional features over thorough documentation
...
- Enables experimentation without compromising documentation outputs
...
| Sprint   | Duration   | Focus Area                        | Key outputs                                                          |
| -------- | ---------- | --------------------------------- | ------------------------------------------------------------------------- |
...
| Sprint 6 | Week 11-12 | Additional Features & Integration | Lecturer features, notifications, reporting                     |
...
- **Who we talked to**: All team members
- **Objectives**:
...
- **Who we talked to**: Team members + people involved (instructor, potential users)
- **Objectives**:
  - Demonstrate completed features
  - Gather feedback from people involved
...
- **Who we talked to**: Team members only
- **Objectives**:
...
- **Who we talked to**: Project Manager + interested team members
- **Objectives**:
...
| Team Member | Primary Role             | Secondary Responsibilities                       | Key outputs                                       |
| ----------- | ------------------------ | ------------------------------------------------ | ------------------------------------------------------ |
...
- help Agile ceremonies
- Remove blockers and impediments
- Track project progress and velocity
- Coordinate communication with people involved
- Manage project timeline and outputs
...
| Email           | Formal communication, people involved | As needed      | Email                              |
...
| Risk ID | Risk Description                                              | Probability | Impact | How We'll Handle It                                     | Contingency Plan                                                    |
| ------- | ------------------------------------------------------------- | ----------- | ------ | ------------------------------------------------------- | ------------------------------------------------------------------- |
...
| R-005   | Database performance issues with test data                    | Low         | Medium | build database indexing, optimize queries           | Use database monitoring tools, add caching layer                    |
| R-006   | Security vulnerabilities                                      | Medium      | High   | Follow OWASP guidelines, conduct security reviews       | Perform security audit, build recommended fixes                 |
